...
describe "Login shells" do
  ...
  it "only `ubuntu` has a login shell" do
    ...
    # Otherise, if we are on Travis CI, allow "henrik", "maria", "me", "solarce", and "travis" users
    elsif ENV["TRAVIS"] == "true"
      ALLOWED_USERS.push("henrik", "maria", "me", "solarce", "travis")
    end
...
describe "ssh" do
  it "is a version patched for malicious UseRoaming" do
    expect(package("openssh-server").version).to(eq("1:6.6p1-2ubuntu2.7"))
  end
end
